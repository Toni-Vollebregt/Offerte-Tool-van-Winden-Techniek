...
function valideerOfferte(klussen: Klus[], offerte: Offerte): ValidationWarning[] {
  ...
  if (klussen0km.length > 0 && klussen0km.length === klussen.length) {
    meldingen.push({
      type: 'warning',
      veld: 'afstandKm',
      bericht: `Alle ${klussen.length} klussen hebben afstand 0 km en reistijd 0 — mogelijk is de Google Maps API niet geconfigureerd.`,
    })
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#2D2D2D' }}>
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 py-8">
        <StepIndicator currentStep={1} flow="offerte" />
        ...
        <div className="mt-8 space-y-8">
          ...
          {validatieResultaat ? (
            ...
            <>
              <PdfUpload onUpload={handleUpload} isLoading={isLoading} error={error} label="maandplanning" />
                ...
                <div className="max-w-2xl mx-auto rounded-xl p-5 space-y-3" style={{ backgroundColor: '#3D3D3D' }}>
                  ...
                  <ol className="space-y-2 text-sm" style={{ color: '#9D9D9D' }}>
                    {[
                      ...
                      'Review en pas aan waar nodig, genereer dan de definitieve offerte PDF',
                    ].map((stap, i) => (
