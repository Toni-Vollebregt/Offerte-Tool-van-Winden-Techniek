...
export default function OfferteReviewPage() {
  ...
  const handleDoorgaan = () => {
    if (!offerte) return
    setIsSaving(true)
    const updatedOfferte: Offerte = { ...offerte, klussen, ...totalen }
    sessionStorage.setItem('offerte', JSON.stringify(updatedOfferte))
    router.push('/offerte/finalize')
  }

  const handleTerug = () => router.push('/offerte')
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#2D2D2D' }}>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 py-8">
        <StepIndicator currentStep={3} flow="offerte" />

        <div className="mt-8 space-y-6">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-2xl font-bold text-white">Offerte controleren</h1>
